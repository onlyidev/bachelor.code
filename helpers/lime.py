...
class LimeExplainer:
    ...
    def __getExplainer(self, data):
        ...
        return LimeTabularExplainer(training_labels=mca_data.values[:, -1], training_data=mca_data.values[:, :-1], feature_names=mca_data.columns[:-1], class_names=['Benign', 'Malicious'], discretize_continuous=True, kernel_width=1)
